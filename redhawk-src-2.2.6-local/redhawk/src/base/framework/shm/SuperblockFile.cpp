...
SuperblockFile::Statistics SuperblockFile::getStatistics()
{
    ...
    size_t offset = sysconf(_SC_PAGESIZE);
    ...
    while (offset < end) {
        ...
        void* base = _file.map(sysconf(_SC_PAGESIZE), MappedFile::READONLY, offset);
        ...
        if (valid) {
            ...
            offset += sysconf(_SC_PAGESIZE) + superblock->size();
        }
        // Don't forget to unmap--it doesn't happen automatically!
        _file.unmap(base, sysconf(_SC_PAGESIZE));
...
void SuperblockFile::create()
{
    ...
    try {
        _file.resize(sysconf(_SC_PAGESIZE));
    ...
    void* base = _file.map(sysconf(_SC_PAGESIZE), MappedFile::READWRITE);
...
void SuperblockFile::open(bool attach)
{
    ...
    if (_file.size() < sysconf(_SC_PAGESIZE)) {
...
void SuperblockFile::close()
{
    ...
    _file.unmap(_header, sysconf(_SC_PAGESIZE));
...
Superblock* SuperblockFile::createSuperblock(size_t bytes)
{
    ...
    size_t total_size = sysconf(_SC_PAGESIZE) + bytes;
...
Superblock* SuperblockFile::_mapSuperblock(size_t offset)
{
    ...
    void* base = _file.map(sysconf(_SC_PAGESIZE), MappedFile::READWRITE, offset);
    ...
    base = _file.remap(base, sysconf(_SC_PAGESIZE), sysconf(_SC_PAGESIZE) + superblock_size);
